...
- **No commit message may mention Claude or Anthropic, in any form** (`Co-Authored-By:
  ...
  Editing the body after that check has already run does **not** clear a failure by itself —
  `pr-checks.yml` triggers on `edited` for exactly this reason (title/body edits alone now
  re-run it against the fixed text), so re-running the same job or leaving the trigger at its
  default `[opened, synchronize, reopened]` would otherwise replay the stale payload and fail
  again even after the footer is gone; two ports (pyvrl, certbot-dns-multi) worked around this
  by closing and reopening their own PR before the `edited` trigger existed — don't do that,
  a body edit now retriggers the check on its own.
