...
307. **A release whose own official wheels are generated by a patched, pinned SWIG fork —
    not whatever SWIG a manylinux image ships — can fail its upstream test suite on
    Python-object-shape questions that have nothing to do with target architecture.**
    CasADi's `misc/Dockerfile.swig` builds `jaeandersson/swig@98c1840366b9...` (not stock
    SWIG) into `ghcr.io/casadi/ci-swig`, and that image is what generates the Python
    bindings for every wheel CasADi itself ships (its own `.github/workflows/binaries.yml`
    `swig` job runs `make python_source` inside it, then flips `SWIG_IMPORT` on so
    downstream release builds skip running SWIG entirely and just import the pre-generated
    sources). Diffing that fork against `swig/swig` shows it patches
    `Source/Modules/python.cxx` to add: a `-stubs` flag that emits `.pyi` files from
    typemap `doc=` annotations (already a known, documented gap in this port — stock SWIG
    has no such flag, see gotcha 289/294's build-casadi.yml), and a
    `%feature("customdoc")`-gated codegen path that changes what happens when every
    overload of a call fails to match: instead of the plain `TypeError` stock SWIG raises,
    it can fall through to returning `Py_NotImplemented` (letting Python's operator
    protocol try the reflected method / synthesize its own message) or merge a base
    class's option docs into a derived factory function's `__doc__`. This port builds
    casadi's plain `git` checkout in the riscv64 manylinux image instead (no riscv64
    dockcross target exists to run the pinned fork through upstream's own cross-compile
    path), so CMake falls back to whatever stock SWIG the image ships, and upstream's own
    `test/python/alltests.py` — written and only ever exercised against ci-swig-generated
    bindings — fails on OUTPUT-typemap tuple-packing shape (`test_get_ccs`,
    `test_simple_bounds_detect`, `test_extract`'s vector-of-MX argout), operator/
    `NotImplemented` fallback (`test_issue4268`, `test_mul3_issue_1465`), and merged
    autodoc content (`test_doc`) — `test_OUTPUT` even branches on a `swig4` flag inside
    the test itself, i.e. upstream already treats this as build-toolchain-dependent, not
    as a fixed contract. None of these touch a computed value — they're all about
    exception type/message or docstring/return-container shape — so a riscv64-only
    numerical or codegen bug was not a plausible explanation once the actual tracebacks
    were read. Deselect them alongside the already-known `test_stubs_installed`: don't
    patch `alltests.py` (it has no CLI test-selection flag of its own and patching a
    project's whole-suite runner is a bigger deviation than needed) — instead, drive
    `unittest.TestLoader`/`TestSuite` directly from a small inline script in the
    workflow's test step that calls the suite's own `build_suite()`, recursively prunes
    the known-excluded test ids, and runs what's left through `TextTestRunner`.

321. **When the excluded boundary is a whole submodule reachable by name, stub its
    `suite()` instead of gotcha 307's flatten-and-prune (the fabio case).** fabio's
    `fabio.test.run_tests()` is a fixed `unittest.TextTestRunner().run(test_all.suite())`
    call with no `-k`/`--deselect` equivalent, and `test_all.suite()` wires in every
    `test_*` submodule by name, including `test_import` — whose one test does
    `__import__` on every `.py` file under the package, among them `fabio/qt/dialogs.py`,
    which needs a real Qt binding (PySide6/PyQt) at import time. No Qt binding publishes a
    riscv64 wheel anywhere, so that import always raises. `test_all.py` reaches the
    submodule through a plain attribute lookup (`from . import test_import`, then
    `test_import.suite()`), so rebinding that one name before building the top-level suite
    drops exactly its tests, with no need to flatten the assembled suite and prune by test
    id:
    ```
    CIBW_TEST_COMMAND: >-
    ...
    ```
    Reach for this only when the whole submodule is the unavailable-dependency boundary —
    one `TestCase` here, no test in it worth keeping (gotcha 212's "check which side of
    the ratio a suite falls on" before picking a tool). A submodule mixing excluded and
    keepable tests still needs gotcha 307's flatten-by-id approach.

339. **An unpinned `pytest` in `CIBW_TEST_REQUIRES` can resolve to a pytest new enough
    to enforce checks a project's own pinned CI never ran — pin to the project's own
    ceiling instead of guessing at a fix.** pytest 9.1 added `strict_parametrization_ids`
    (an error, not a warning, on duplicate `@pytest.mark.parametrize`/fixture IDs). It's
    off by default, but a project whose own `pyproject.toml` sets `addopts = "--strict"`
    (the old bundle flag) turns it on — and pytest's config-file search climbs from the
    test path upward through every parent directory looking for one, so it can find that
    `pyproject.toml` even when testing the *installed wheel*, not the checkout: cibuildwheel
    stages the built sdist and the test venv side by side under one `/tmp/tmp.<hash>/`
    working directory, so `.../venv/lib/pythonX.Y/site-packages/<pkg>` still climbs to the
    sdist's own `pyproject.toml` a few levels up. Both statsmodels and arch hit this exact
    error — `Duplicate parametrization IDs detected, but strict_parametrization_ids is set`
    — on a duplicate ID their own test suite has carried for a while, never caught because
    their real CI pins `pytest<9`/`pytest<9.1` (check the project's `pyproject.toml`/CI
...
350. **A suite's own `try: import X except ImportError: X = None` plus
    `@pytest.mark.skipif(not X)` guard turns an omitted `CIBW_TEST_REQUIRES` entry into a
    quiet skip, not a failure — the job goes green with a fraction of the coverage
    (the pyopengl-accelerate case).** `CIBW_TEST_REQUIRES: pytest PyOpenGL` (numpy left
    out, since it wasn't in the runtime dependency list) still passed: 20 passed, 33
    skipped, every skip reading `Numpy not available`. Nothing in the run's exit code or
    `pass`/`fail` summary flags this as a problem — pytest's `-ra` summary shows the
    skip reasons, but only if you read past the top-line pass count rather than trusting
    a green job. This is the inverse of gotcha 212 (an unavailable dependency masking as
    an `ImportError` crash): here the dependency *is* available (numpy has a riscv64
    wheel on our own registry), it was just never asked for, and the project's own
    defensive `try/except ImportError` — written so upstream's suite still runs without
    the optional accelerator — absorbs the gap instead of surfacing it. Check a build's
    `-ra` short summary (or grep for `SKIPPED` counts) against what the local dry run
