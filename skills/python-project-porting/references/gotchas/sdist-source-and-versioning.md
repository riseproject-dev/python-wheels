...
521. **A SWIG/autotools binding can publish wheels and *no* sdist, because its real build
     input is upstream's `make dist` tarball — and the git tree does not contain the generated
     wrapper at all (the quantlib case).** `lballabio/QuantLib-SWIG` has `Python/setup.py` whose
     only `Extension` source is `src/QuantLib/quantlib_wrap.cpp`, and that file is not in git:
     `Python/Makefile.am` lists it under `BUILT_SOURCES` and `EXTRA_DIST`, so it is generated by
     SWIG during `make dist` and exists only inside the release tarball. A git checkout or a tag
     archive therefore has nothing to compile, `pip download --no-binary` finds no sdist on PyPI
     at all, and gotcha 2's "fetch the sdist and inspect it" has no artifact to work with.
     - **Mirror upstream's tarball job rather than wiring in the release asset.** Upstream's own
       `wheels.yml` opens with a `make-tarballs` job — install SWIG from source at the exact
       version it pins, then `./autogen.sh && ./configure && make dist` in *both* the library
       repo and the bindings repo — precisely because the tarballs are the build input. That job
       is architecture-independent, so it runs once on `ubuntu-latest` (gotcha 4) and uploads
       both tarballs as the port's `<pkg>-<ver>-sdist` artifact. This satisfies the
       "build the sdist yourself from an upstream checkout" rule without inventing a shape:
       the shape is upstream's.
     - **The bindings' `configure` does not need the library installed.** It shells out to
       `quantlib-config --version` with no `AC_MSG_ERROR` behind it, so `make dist` succeeds
       against a machine that has only SWIG and the dep's headers — which is why the tarball
       job needs no C++ build of its own.
     - **A two-tarball layout still works with `package-dir` pointing inside one of them.**
       cibuildwheel copies `Path.cwd()` (not `package-dir`) to `/project` and runs `before-all`
       there (gotcha 142), so `package-dir: ./<Bindings>-<ver>/Python` leaves upstream's
       `CIBW_BEFORE_ALL_LINUX` script free to `cd ../<Library>-<ver>/` and build the C++ library
       next door, and free to live in the bindings *git* checkout (`.ci/wheels/*.sh`, which the
       dist tarball does not ship). Check both repos out, download the tarball artifact, unpack
       both at the workspace root, and upstream's own script runs unmodified.
     - **Assert the version instead of globbing.** Upstream's job moves `QuantLib-*.tar.gz`;
